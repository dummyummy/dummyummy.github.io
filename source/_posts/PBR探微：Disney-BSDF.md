...
为了搞懂这个式子，先关注后面的部分，$G$和$D$的乘积代表了可见的微观法线朝向为$\mathrm{m}$的微平面面积之和占宏观面积的比例，这点很好理解。
...
$$
f_t(\mathrm{i},\mathrm{o},\mathrm{n})=\left| \dfrac{\mathrm{i} \cdot \mathrm{h}_t}{\mathrm{i} \cdot \mathrm{n}} \right| \left| \dfrac{\mathrm{o} \cdot \mathrm{h}_t}{\mathrm{o} \cdot \mathrm{n}} \right| \dfrac{\eta_o^2(1-F(\mathrm{i},\mathrm{h}_t))G(\mathrm{i},\mathrm{o},\mathrm{h}_t)D(\mathrm{h}_t)}{(\eta_i(\mathrm{i} \cdot \mathrm{h}_t)+\eta_o(\mathrm{o} \cdot \mathrm{h}_t))^2}  \tag{9}
$$
...
另外注意由于我们采样的是$\mathrm{h}$，最终的结果还要乘上基本定义中Reflection BRDF一节推导出的雅可比行列式才能得到正确的概率。
...
$$
\Lambda(\mathrm{o})=\frac{-1+\sqrt{1+1/a^2}}{2}
$$
...
\end{aligned}
$$
然后很巧的是，在(10)式的分母上正好有一个$(\mathrm{o} \cdot \mathrm{n})$，为了数值稳定性和效率考虑，Disney在[8]的实现中将$2(\mathrm{o} \cdot \mathrm{n})$与$G_1$项进行了合并，这样就得到了：
$$
\begin{aligned}
...
TODO：根据[12]记录一下Shadow Masking Function的本质
...
Disney在论文中提到，由于会与清漆发生交互的光线能量总占比是很少的，同时为了尽可能保持计算的简单，他们将`clearcoat`缩放到了[0,0.25]范围后，直接把clearcoat lobe叠加到了diffuse lobe上。这样理论上是能量不守恒的，但是视觉上能说的过去。
...
![output](PBR探微：Disney-BSDF/output.png)
...
- 在试验时发现，完美玻璃的BSDF如果渲染方程中仍然乘上余弦项的话会导致边缘异常变暗，分析原因如下，当roughness为0时，NDF会退化为是一个Dirac函数，带入到Reflection BRDF中可以得到此时宏观的BRDF也是(2)式（很合理），渲染方程中的几何项会被抵消。然后从能量守恒的角度出发，可知折射的能量一定是1-F，侧面说明此时折射的BTDF的分母也能抵消几何项。
